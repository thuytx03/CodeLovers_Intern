            ...
            <form action="{{ route('update.product', ['id' => $product->id]) }}" method="POST" enctype="multipart/form-data">
                                ...
                                @foreach ($data->where('parent_id', 0) as $item)
                                    <div class="form-check">
                                        <input class="form-check-input" name="category_id[]" type="checkbox"
                                            ...
                                            @if(in_array($item->id, $product_category->pluck('category_id')->toArray() )) checked  @endif
                                            >
                                    ...
                                    @foreach ($data->where('parent_id', $item->id) as $children)
                                        <div class="form-check">
                                            <input class="form-check-input" name="category_id[]" type="checkbox"
                                                value=" {{ $children->id }}" id="flexCheckDefault" style="margin-left:5px;" @if(in_array($children->id, $product_category->pluck('category_id')->toArray() )) checked  @endif
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="file" name="image" id="image" class="form-control"
                            value="">
                        <img id="preview-image" src="{{ Storage::url($product->image) }}" alt="Ảnh sản phẩm"
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10 image-container">
                        <input type="file" name="images[]" id="images" class="form-control" value="" multiple>
                    </div>
                    @foreach ($images as $image)
                        <img src="{{ Storage::url($image->image_path) }}" alt="Ảnh sản phẩm" style="max-width: 200px;">
                    @endforeach
                </div>
                <div class="row mb-3">
                ...
                <div id="inputsContainer" >
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="discount_price" class="form-control"  value="{{  $product->percent==NULL? $product->discount_price: ''  }} ">
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label for="inputText" class="col-sm-2 col-form-label">Giảm theo phần trăm</label>
                        <div class="col-sm-10">
                            <input type="text" name="percent" class="form-control" value="{{ $product->percent ? $product->percent : NULL }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-sm-10 ">
                        @foreach ($size as $item)
                            <input class="form-check-input" name="size_id[]" type="checkbox"
                                ...
                                @if(in_array($item->id, $product_size->pluck('size_id')->toArray() )) checked  @endif
                                >
                    ...
                    <div class="col-sm-10 ">
                        @foreach ($color as $item)
                            <input class="form-check-input" name="color_id[]" type="checkbox"
                                value="{{ $item->id }}" id="gridCheck1"
                                @if(in_array($item->id, $product_color->pluck('color_id')->toArray() )) checked  @endif
                                >
                            <label class="form-check-label" for="gridCheck1">
                                {{ $item->name }}
                            </label>
