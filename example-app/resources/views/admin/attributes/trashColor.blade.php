...
@section('content')
    ...
    <form action="{{ route('deleteAllColor.attributes') }}" method="post" id="delete-form">
        <div>
            <h5 class="mt-3 mb-3">Quản lý màu sắc sản phẩm</h5>
    ...
    <form action="" method="post" id="delete-form">
        <div>
            <h5 class="">Quản lý kích cỡ sản phẩm</h5>
            <div class="d-flex justify-content-end mt-3 mb-3 ">
                ...
                @foreach ($colors as $key => $item)
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="/admin/roles/destroy/{{ $item->id }}"
                                onclick="event.preventDefault(); showDeleteConfirm('{{ $item->id }}','{{ $role }}');"
                                class="btn btn-danger mt-1"><i class='bx bx-trash'></i></a>

                            <a href="/admin/roles/edit/{{ $item->id }}" class="btn btn-primary">Sửa</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
